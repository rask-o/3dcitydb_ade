...
----------------------------------------------------------------
-- Function NRG8_INSERT_ELECTRICITY_EXCHANGER
----------------------------------------------------------------
-- DROP FUNCTION IF EXISTS citydb_pkg.nrg8_insert_electricity_exchanger (integer, integer, integer, integer, character varying) CASCADE;
CREATE OR REPLACE FUNCTION citydb_pkg.nrg8_insert_electricity_exchanger (
  objectclass_id       integer,
  id                   integer,
  network_id           integer DEFAULT NULL,
  network_node_id      integer DEFAULT NULL,
  prim_electricity_supplier character varying DEFAULT NULL,
  schema_name          varchar DEFAULT 'citydb'::varchar
)
RETURNS integer
AS
...
DECLARE
  ...
  p_network_id         integer;
  p_network_node_id    integer;
  p_prim_electricity_supplier character varying;
--
  p_schema_name        varchar;
  inserted_id integer;
BEGIN
-- Pass values to internal prefixed variables to avoid potential homonymy issues
  ...
  p_network_id         :=network_id;
  p_network_node_id    :=network_node_id;
  p_prim_electricity_supplier :=prim_electricity_supplier;
  p_schema_name        :=schema_name;


EXECUTE format('
    INSERT INTO %I.nrg8_electricity_exchanger (
     id,
     objectclass_id,
     network_id,
     network_node_id,
     prim_electricity_supplier
    ) VALUES (
    %L, %L, %L, %L, %L
    ) RETURNING id',
    p_schema_name,
    ...
    p_network_id,
    p_network_node_id,
    p_prim_electricity_supplier
) INTO inserted_id;
RETURN inserted_id;
EXCEPTION
	WHEN OTHERS THEN RAISE NOTICE 'citydb_pkg.nrg8_insert_electricity_exchanger (id: %): %', p_id, SQLERRM;
END;
$$
LANGUAGE 'plpgsql';

----------------------------------------------------------------
-- Function NRG8_INSERT_GAS_EXCHANGER
----------------------------------------------------------------
-- DROP FUNCTION IF EXISTS citydb_pkg.nrg8_insert_gas_exchanger (integer, integer, integer, integer, character varying) CASCADE;
CREATE OR REPLACE FUNCTION citydb_pkg.nrg8_insert_gas_exchanger (
  objectclass_id       integer,
  id                   integer,
  network_id           integer DEFAULT NULL,
  network_node_id      integer DEFAULT NULL,
  prim_gas_supplier character varying DEFAULT NULL,
  schema_name          varchar DEFAULT 'citydb'::varchar
)
RETURNS integer
AS
...
DECLARE
  ...
  p_network_id         integer;
  p_network_node_id    integer;
  p_prim_gas_supplier character varying;
--
  p_schema_name        varchar;
  inserted_id integer;
BEGIN
-- Pass values to internal prefixed variables to avoid potential homonymy issues
  ...
  p_network_id         :=network_id;
  p_network_node_id    :=network_node_id;
  p_prim_gas_supplier :=prim_gas_supplier;
  p_schema_name        :=schema_name;


EXECUTE format('
    INSERT INTO %I.nrg8_gas_exchanger (
     id,
     objectclass_id,
     network_id,
     network_node_id,
...
RETURN inserted_id;
EXCEPTION
	WHEN OTHERS THEN RAISE NOTICE 'citydb_pkg.nrg8_insert_gas_exchanger (id: %): %', p_id, SQLERRM;
END;
$$
LANGUAGE 'plpgsql';

----------------------------------------------------------------
-- Function NRG8_INSERT_OIL_EXCHANGER
----------------------------------------------------------------
-- DROP FUNCTION IF EXISTS citydb_pkg.nrg8_insert_oil_exchanger (integer, integer, integer, integer, character varying) CASCADE;
CREATE OR REPLACE FUNCTION citydb_pkg.nrg8_insert_oil_exchanger (
  objectclass_id       integer,
  id                   integer,
  network_id           integer DEFAULT NULL,
  network_node_id      integer DEFAULT NULL,
  prim_oil_supplier character varying DEFAULT NULL,
  schema_name          varchar DEFAULT 'citydb'::varchar
)
RETURNS integer
AS
...
DECLARE
  ...
  p_network_id         integer;
  p_network_node_id    integer;
  p_prim_oil_supplier character varying;
--
  p_schema_name        varchar;
  inserted_id integer;
BEGIN
-- Pass values to internal prefixed variables to avoid potential homonymy issues
  ...
  p_network_id         :=network_id;
  p_network_node_id    :=network_node_id;
  p_prim_oil_supplier :=prim_oil_supplier;
  p_schema_name        :=schema_name;


EXECUTE format('
    INSERT INTO %I.nrg8_oil_exchanger (
     id,
     objectclass_id,
     network_id,
     network_node_id,
...
RETURN inserted_id;
EXCEPTION
	WHEN OTHERS THEN RAISE NOTICE 'citydb_pkg.nrg8_insert_oil_exchanger (id: %): %', p_id, SQLERRM;
END;
$$
LANGUAGE 'plpgsql';
