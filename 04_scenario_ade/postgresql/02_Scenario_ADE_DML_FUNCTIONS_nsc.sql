...
EXECUTE format('SELECT citydb_pkg.scn2_delete_operation(op.id, %L), COUNT(*) FROM %I.scn2_operation op JOIN %I.scn2_scenario_to_operation sto ON sto.operation_id = op.id WHERE sto.scenario_id = %L GROUP BY op.id HAVING COUNT(*) = 1', schema_name, schema_name, schema_name, o_id);
-- Delete relation scenario_to_operation (has to be done in case operation is not deleted by previous command, which would cascade to the relation)
EXECUTE format('DELETE FROM %I.scn2_scenario_to_operation WHERE scenario_id = %L', schema_name, o_id);
